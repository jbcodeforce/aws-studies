...
Since 2006, Security is our number 1 priority, When you work with the AWS Cloud, managing security and compliance is a [shared responsibility](https://aws.amazon.com/compliance/shared-responsibility-model/) between AWS and you:

* [AWS is responsible of security **of** the cloud](https://aws.amazon.com/security/) and offers the most flexible and secure cloud computing environment available today. AWS is responsible for patching managed services.
* You are responsible for the security **in** the cloud: secure workloads and applications that you deploy onto the cloud. When using EC2, you are responsible to patch OS for security (but AWS helps by providing patched AMIs, or tools such as [Systems Manager]() as your operations hub for app and AWS resources you use, or [Inspector](https://aws.amazon.com/inspector/) for continuous vulnerability testing).
...
For highly regulated industry, AWS helps by getting more than 50 certifications for the infrastructure, globally but also regionaly for specific country. At global, [ISO 27001](https://www.iso.org/isoiec-27001-information-security.html) SOC 1 and 2 certifications. For regional in the USA, AWS aligns with [FISMA](https://www.cisa.gov/federal-information-security-modernization-act) and [FedRAMP](https://www.gsa.gov/technology/government-it-initiatives/fedramp), in UK AWS aligns with Cyber Essentials, in Australia with IRA. The audits are done by a 3nd party and can reports are in [aws/artifact](https://aws.amazon.com/artifact/).
...
Fine-grain identity and access controls combined with continuous monitoring for near real-time security information ([CloudTrail](https://aws.amazon.com/cloudtrail/)) ensures that the right resources have the right access at all times, wherever your information is stored

## Encryption

Encryption is widely available through a lot of services and features on top of the platform. You will be able to develop application that can encrypt data at rest, or in transit as it flows over the network between services. S3 storage or EBS block attached storage, has single click option to do encryption at rest with keys we are managing or using KMS keys.

![](./images/EBS-encryption-rest.png)

The managed service, [AWS Key Management Service](https://aws.amazon.com/kms/), helps you centrally managing your own keys. It is integrated into a lot of services and your keys never leave AWS [FIPS 140-validated](https://en.wikipedia.org/wiki/FIPS_140-2) Hardware Security Modules unencrypted. You control access and use of the keys.

## Organization

[AWS Organization]((https://us-east-1.console.aws.amazon.com/organizations) helps to centraly manage multiple AWS accounts, group accounts, and simplify account creation. Consolidate billing.
...
* Define users (physical person), groups and roles, and permissions (policies)
...
* get user as administrator, meaning it will be part of an admin group with admin priviledges, like `AdministratorAccess`
...
We can test Policies with the [policty simulator](https://policysim.aws.amazon.com/home/index.jsp?#)

### MFA

* Multi Factor Authentication used to verify a human is using the service
...
* To get AWS services doing work on other service, use IAM Role. Roles are assigned per application, or EC2 or lambda function...
...
???- "Sensitive data in the cloud can be accessed by everyone"
    WAS storage managed services are secured by default. You control who can access your data. For example, in S3 bucket, objects, by default are only available by the owner who created them. IAM policies define access to buckets, and in S3 itself you can define policies. Some policies can be defined at the Account level to forbid to get define public bucket. From detection services like CloudTrail and CloudWatch Events you can route events to security team or respond in real-time with code in a Lambda function.
...
???- "How to be sure to data is really deleted"
    You own the data inside AWS. But on delete operation the data is destroyed and wiped out. When destroying physical hardware, AWS follows the NIST 888 requirements, and it is done in a secure zone in the data center. Audits are done by 3nd party vendor, and compliances are reported in `Artifact`.
